                                ...
                                .then(
                                    Commands.argument("team_id", StringArgumentType.word()) // Аргумент 2: КОМАНДА
                                        .executes { ctx ->
                                            ...
                                            if (lobby == null) {
                                                ctx.source.sendFailure(text("player_not_in_lobby", TextType.ERROR, args = arrayOf(targetPlayer.name.string)))
                                                return@executes 0
                                            }

                                            val teamExists = lobby.currentMap.teams.any { it.teamId == newTeamId }
                                            if (!teamExists) {
                                                ctx.source.sendFailure(text("team_is_not_exist", TextType.ERROR, args = arrayOf(newTeamId)))
                                                return@executes 0
                                            }
                                            ...
                                            ctx.source.sendSuccess({ text("set_player_team", TextType.SUCCESS, args = arrayOf(targetPlayer.name.string, newTeamId)) }, true)
                                            1
                                        }
                                )
                        )
