...
object ScoreboardManager {
    ...
    private val playerActiveLines = HashMap<UUID, MutableList<String>>()
    ...
    fun initScoreboard(player: ServerPlayer, title: Component) {
        ...
        player.connection.send(ClientboundSetDisplayObjectivePacket(1, objective))
    }

    fun updateScoreboard(player: ServerPlayer, lobby: LobbyInstance) {
        val playerUuid = player.uuid
        val oldLines = playerActiveLines[playerUuid]
        ...
        if (oldLines != null) {
            ...
            playerActiveLines[playerUuid] = ArrayList()
        }
    ...
    fun removeScoreboard(player: ServerPlayer) {
        ...
        playerActiveLines.remove(player.uuid)// 1 = Удалить
    }
}